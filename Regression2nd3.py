...
for i in range(226):
    for j in range(226):
        tmp1=trainData[:,i].reshape(-1,1)
        tmp2=trainData[:,j].reshape(-1,1)
...
for i in range(226):
    ...
    if tmpVar<0.1:
        tmpVar=1.0;
    #trainData=np.concatenate((trainData, np.exp(-(tmp-tmpMean)*(tmp-tmpMean)/(2*tmpVar))/np.sqrt(tmpVar*2*PI)),axis=1)
    trainData = np.column_stack((trainData, np.exp(-(tmp-tmpMean)*(tmp-tmpMean)/(2*tmpVar))/np.sqrt(tmpVar*2*PI)))
...
reg=Ridge(alpha=0.01,copy_X=False,solver='lsqr')
...
for i in range(226):
    for j in range(226):
        tmp1=testData[:,i].reshape(-1,1)
        tmp2=testData[:,j].reshape(-1,1)
        testData=np.concatenate((testData,tmp1*tmp2),axis=1)
        print('('+str(i)+','+str(j)+')')
#加高斯核
for i in range(226):
    ...
    if tmpVar<0.1:
        tmpVar=1.0;
    testData=np.concatenate((testData, np.exp(-(tmp-tmpMean)*(tmp-tmpMean)/(2*tmpVar))/np.sqrt(tmpVar*2*PI)),axis=1)
